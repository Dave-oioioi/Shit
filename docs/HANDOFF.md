...
- `docs/handoff-v1.0-release.md`
  - Current release handoff for SHIT VAULT `v1.0.0`.
  - Covers single-instance runtime protection, single install mode, installer build, release asset, verification, and remaining release/upload status.
...
SHIT VAULT is being closed out as `v1.0.0`: a Windows tray-first desktop utility with protected shell behavior, frozen `prevent-sleep`, completed `auto-mixing`, single-instance runtime enforcement, one current-user NSIS install line, and a GitHub Release installer artifact.
...
- Previous published release: `v0.1.0`
- Target release page: `https://github.com/Dave-oioioi/SHIT/releases/tag/v1.0.0`
- Local installer path: `src-tauri/target/release/bundle/nsis/SHIT VAULT_1.0.0_x64-setup.exe`
...
- `src/modules/auto-mixing/`
  - current 1.0 module UI/state/settings behavior
...
After local verification, publish `v1.0.0`:

1. Commit the release changes.
2. Tag `v1.0.0`.
3. Push `main` and tag.
4. Upload `SHIT VAULT_1.0.0_x64-setup.exe` to GitHub Release `v1.0.0`.
